...
public class UserLoginRegistrationService {
    ...
    private final UserServiceExternalAPIs userServiceExternalAPIs;


    public UserAttributesModel registerNewUser(@Valid UserRegistrationModel userModel) {
        UserCompanyModel userFetchedFromCompanyDB = userServiceExternalAPIs.fetchUserInfoFromCompanyDB(userModel.getId());
         ...
         if(userFetchedFromCompanyDB.getSsn().equals(userModel.getSsn())&&
            ...
            UserAttributesModel userAttributesModel = userServiceExternalAPIs.fetchUserAttributesFromCompanyDB(userModel.getId());
            userServiceExternalAPIs.saveNewUserCredentials(registrationModel);
            userServiceExternalAPIs.saveNewUserAttributes(userAttributesModel);
    ...
    public UserAttributesModel validateExistingUser(UserLoginModel userLoginModel) {
        ...
        UserRegistrationModel userRegistrationModel = userServiceExternalAPIs.fetchUserCredentials(userLoginModel.getId());
        if(userRegistrationModel != null){
            ...
            if (isPwdRight) {
                return userServiceExternalAPIs.fetchUserAttributes(userRegistrationModel.getId());
            }else
